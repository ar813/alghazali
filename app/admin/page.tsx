...
const AdminPortal = ({ isBlurred = false, onLoadingChange }: { isBlurred?: boolean; onLoadingChange?: (loading: boolean) => void }) => {
    const [activeTab, setActiveTab] = useState<'dashboard' | 'students' | 'schedule' | 'reports' | 'cards' | 'fees' | 'notice' | 'quiz' | 'results' | 'examResults'>('dashboard');
    const [collapsed, setCollapsed] = useState(false);

    const sidebarItems: { id: 'dashboard' | 'students' | 'schedule' | 'reports' | 'cards' | 'fees' | 'notice' | 'quiz' | 'results' | 'examResults'; label: string; icon: any }[] = [
    ...
    useEffect(() => {
        const applyHash = () => {
            ...
            if (hash && ['dashboard','students','schedule','reports','cards','fees','notice','quiz','results','examResults'].includes(hash)) {
