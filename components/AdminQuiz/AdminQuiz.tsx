...
const AdminQuiz = ({ onLoadingChange }: { onLoadingChange?: (loading: boolean) => void }) => {
  ...
  const submit = async () => {
    ...
    if (!form.examKey.trim()) return toast.error('Exam Key is required')
    if (form.targetType === 'class' && !form.className) return toast.error('Class is required')
    if (form.targetType === 'student' && !form.studentId) return toast.error('Student is required')
    if (typeof form.questionLimit !== 'number' || !(form.questionLimit >= 1 && form.questionLimit <= 200)) return toast.error('Total Questions is required (1-200)')
    ...
    if (!valid) return toast.error('Please fill all questions with 4 options and a correct index')
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => {
                      ...
                      } catch { toast.error('Failed to export questions') }
