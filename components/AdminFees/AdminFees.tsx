...
const AdminFees = ({ onLoadingChange }: { onLoadingChange?: (loading: boolean) => void }) => {
  ...
  const handleExportFees = async () => {
    try {
      if (fees.length === 0) {
        toast.error('Koi fees records nahi hain export karne ke liye!')
        return
      }
      ...
      toast.error(`Excel export mein error: ${error?.message || 'Unknown error'}`)
    }
  }
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await loadFees()
      toast.success(`${okCount} out of ${toCreate.length} fees imported successfully.`)
    } catch (error: any) {
      console.error('Excel import error:', error)
      toast.error(`Excel import mein error: ${error?.message || 'Unknown error'}`)
    } finally {
  ...
  const submitForm = async () => {
    ...
    if (!form.studentId) return toast.error('Student is required')
    if (!form.className || String(form.className).trim() === '') return toast.error('Class is required')
    if (!form.month) return toast.error('Month is required')
    if (!form.year && form.year !== 0) return toast.error('Year is required')
    const amt = Number(form.amountPaid as any)
    if (isNaN(amt)) return toast.error('Amount Paid is required')
    if (amt <= 0) return toast.error('Fee cannot be zero')
    if (!form.paidDate || String(form.paidDate).trim() === '') return toast.error('Paid Date is required')
    if (!form.receiptNumber || String(form.receiptNumber).trim() === '') return toast.error('Receipt Number is required')
    if (!form.bookNumber || String(form.bookNumber).trim() === '') return toast.error('Book Number is required')
    setSubmitting(true)
    try {
      const payload: any = {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      await loadFees()
      toast.success(`Deleted ${json.deleted || 0} fees`)
    } catch (e: any) {
